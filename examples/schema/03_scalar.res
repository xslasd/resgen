...
# 公共定义（ResData、IntTime）已在 00_common.res 中声明，此处直接使用

# 使用标量作为模型字段
type Event {
    id:        Int!
    name:      String!
    startTime: IntTime!   # 业务层自动映射为实现类型（如 time.Time 或 scalars.IntTime）
    endTime:   IntTime
    createdAt: IntTime
}
...
input CreateEventInput {
    name:      String!
    startTime: IntTime!   # 请求 Body 中的标量字段
    endTime:   IntTime!
}

input QueryEventsInput {
    after:  IntTime   # Query 参数中的标量（GET 请求自动展开为查询参数）
    before: IntTime
    page:   Int
    size:   Int
}
