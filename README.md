...
- **🌟 契约与文档双轨自动化交付**：编译期不仅生成 Go 强类型代码，更全自动提取注释、权限与约束生成 `docs/api.json` 与 `api.html`；通过 `go:embed` 零代码自动挂载 `GET /docs` 交互式预览，实现开发与文档交付的真正零时差。
...
> 💡 **类型修饰符与指针映射最佳实践**：
> - 未标 `!` 的字段在 Go 侧映射为指针类型（如 `*string`、`*[]string`），标 `!` 的字段映射为值类型（如 `string`、`[]string`）。
> - **切片推荐铁律（内部永远加感叹号）**：
>   - **必填列表（如批量删除）**：永远使用 **`[Type!]!`** 映射为 `[]type`（干净纯粹的切片，必传且非空）；
>   - **可选列表（如筛选条件/配置）**：永远使用 **`[Type!]`** 映射为 **`*[]type`**（外层指针用于区分未传与传 `[]` 清空配置，传 `[]` 不会误报非空）。
>   - 避免使用 `[Type]!`，以免生成包含指针元素的 `[]*type`（前端难以传参且业务需繁琐解引用）。详见 [DSL 语法指南](./docs/dsl_guide.md#类型修饰符与指针映射)。
